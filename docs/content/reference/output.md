...
describes what users get. Pick a format with `-o`, or let google choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
google <command> -o table   # aligned columns for reading
google <command> -o jsonl   # one JSON object per line, for piping
google <command> -o json    # a single JSON array
google <command> -o csv     # spreadsheet friendly
google <command> -o tsv     # tab-separated
google <command> -o url     # just the URL column
google <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
google <command> --fields id,title,url
```
...
```bash
google <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
google <command>            # a table, because this is a terminal
google <command> | wc -l    # JSONL, because this is a pipe
```
